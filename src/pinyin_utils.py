...
def pinyin_to_hidden_pinyin(full_pinyin: str, reveal_mode: str = "first_char_each_syllable") -> str:
    ...
    Default mode 'first_char_each_syllable' (User's rule):
      First letter of EACH syllable is revealed, remaining letters are underscores.
...
def prepare_word_tuple(hanzi: str, custom_pinyin: str = None, custom_hidden: str = None) -> Tuple[str, str, str]:
    """
    Returns (hanzi, full_pinyin, hidden_pinyin)
    Automatically computes pinyin and hidden pinyin if not provided.
    """
    hanzi = hanzi.strip()
...
if __name__ == "__main__":
    test_words = ["苹果", "老师", "学校", "喜欢", "中国", "朋友"]
    for w in test_words:
        h, fp, hp = prepare_word_tuple(w)
        print(f"Hanzi: {h} -> Full: '{fp}' -> Hidden: '{hp}'")

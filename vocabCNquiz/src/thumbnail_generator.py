...
def create_high_ctr_thumbnail(batch_data: Dict[str, Any], output_path: Optional[str] = None) -> str:
    """
    Generate High-CTR 1080x1920 9:16 Vertical Cover Thumbnail for VocabCNQuiz:
    - Top: Level Badge (Emerald Green)
    - Middle: Topic in Vietnamese / Chinese
    - Bottom: 'ĐOÁN NGHĨA TRONG 5S' Badge
    """
    width = 1080
    height = 1920
    
    raw_topic = batch_data.get("topic", "TỪ VỰNG HẰNG NGÀY")
    ...
    draw_rounded_rect(
        ...
        (bx1, by1, bx2, by2),
        radius=50,
        fill=(16, 185, 129, 230),  # Emerald 500
        outline="#34d399",
        width=3
    )
    ...
    # 3. BOTTOM: 'ĐOÁN NGHĨA TRONG 5S' Badge
    bottom_badge_y = 1520
    ...
    draw_rounded_rect(
        ...
        (bbx1, bby1, bbx2, bby2),
        radius=60,
        fill=(5, 150, 105, 245),  # Emerald 600
        outline="#6ee7b7",
        width=3
    )
    draw_text_with_shadow(
    ...
    logger.info(f"Generated High-CTR VocabCN Thumbnail: {output_path}")
    return output_path

/**
 * Social Media Metadata Generator (YouTube Shorts, TikTok, Facebook Reels) for VocabCNQuiz
 */
...
export function generateSocialMetadata(topic, level, words = []) {
  ...
  let ytTitle = `Thử Thách Đoán Nghĩa ${level}: ${cleanTopic} ${chosenEmoji} #Shorts`;
  if (ytTitle.length > 95) {
    ytTitle = `Đoán Nghĩa Tiếng Trung ${level}: ${cleanTopic} ${chosenEmoji} #Shorts`;
  }

  const wordLines = (words || []).map((w, idx) => `${idx + 1}. ${w.hanzi} (${w.pinyin}) ➔ ${w.meaning}`);
  const wordsFormatted = wordLines.join("\n");

  const ytDescription = `🎯 Thử thách đoán nghĩa Tiếng Trung ➔ Tiếng Việt ${level} chủ đề: ${cleanTopic.toUpperCase()}!
...
#lelehoctiengtrung #hoctiengtrung #tiengtrunggiaotiep #vocabquiz #${levelTag.toLowerCase()} #Shorts #learnchinese #hsk`;
  ...
  const tiktokCaption = `Đố bạn dịch đúng 5/5 từ tiếng Trung chủ đề ${cleanTopic} này? ${chosenEmoji} Thử ngay xem bạn được mấy điểm nhé! 💬👇 #lelehoctiengtrung #hoctiengtrung #tiengtrung #${levelTag.toLowerCase()} #vocabquiz #xuhuong #learnchinese`;

  const fbCaption = `Thử tài phản xạ dịch Tiếng Trung sang Tiếng Việt ${level} - Chủ đề ${cleanTopic}! ${chosenEmoji}\nBạn tự tin đúng bao nhiêu câu? Comment kết quả bên dưới cùng Lê Lê nha! ✨\n\n#lelehoctiengtrung #tiengtrung #hoctiengtrung #${levelTag.toLowerCase()} #reelsvn #hsk`;
